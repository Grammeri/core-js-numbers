...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  // Return the angle in radians directly
...
function getParallelepipedDiagonal(a, b, c) {
  ...
  const diagonalSquared = a * a + b * b + c * c;
...
function roundToPowerOfTen(num, pow) {
  const shifted = Math.round(num / 10 ** pow);
  return shifted * 10 ** pow;
}
...
function getSumOfDigits(num) {
  return num <= 9 ? num : (num % 10) + getSumOfDigits(Math.floor(num / 10));
}
...
function isPowerOfTwo(num) {
  return num <= 1 ? num === 1 : isPowerOfTwo(num / 2);
}
